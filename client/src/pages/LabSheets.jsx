...
export default function LabSheets() {
  const { user } = useAuth();
  ...
  const sorted = useMemo(() => {
    if (!data || !data.labsheets) return [];
    return [...data.labsheets].sort((a, b) => new Date(a.deadline) - new Date(b.deadline));
  }, [data]);
  ...
  return (
    <div className="flex min-h-screen" id="labsheets-page">
      <Sidebar />
      <main className="flex-1 ml-64 p-8">
        <div className="mb-8 fade-in">
          <h1 className="text-2xl font-bold">
            <span className="gradient-text">Lab Sheets</span> Progress
          </h1>
          <p className="text-text-muted text-sm mt-1">Keep track of your lab submissions</p>
        </div>

        <div className="glass-card p-6">
          <h3 className="text-sm font-semibold text-text-secondary uppercase tracking-wider mb-4">
            🔬 Lab Sheets ({sorted.length})
          </h3>
          {sorted.length === 0 ? (
                ...
                return (
                  ...
                  >
                    <div className="min-w-0 flex-1">
                      <p className="text-base font-medium truncate">{ls.title}</p>
                      <p className="text-sm text-text-secondary mt-1">{ls.subject_name}</p>
                    </div>
                    <div className="text-right ml-4 shrink-0">
                      ...
                      <p className="text-xs text-text-muted mt-1">
                        {new Date(ls.deadline).toLocaleString()}
                      </p>
                    </div>
                  </div>
                );
  ...
  return (
    <div className="min-h-screen flex items-center justify-center">
      <div className="text-center">
        ...
        <p className="text-text-muted text-sm">Loading lab sheets...</p>
      </div>
    </div>
  );
}
